...
**QwenEncoder is a thin HTTP client** (`qwen/encoder.py`). `t2s()` dispatches on the voice: a preset speaker POSTs JSON to `{url}/v1/audio/speech`; a clone POSTs multipart (the reference audio file) to `{url}/v1/audio/speech/upload`. Both write the raw response bytes to the fragment file and are wrapped in a `tenacity` retry. Server base URL comes from `--qwen-url` (default `http://127.0.0.1:4123`, env `ZAPHODVOX_QWEN_URL`).

**Voice model.** `Voice` (`voice.py`) is an empty Pydantic base; `QwenVoice` (`qwen/voice.py`) subclasses it. A voice is either a **preset** (`voice_id`, e.g. `"Ryan"`, with optional `instruct` emotion/style and `language`) or a **clone** (`ref_audio` path, optional `ref_text` transcript for higher-quality ICL mode vs. zero-shot). A `model_validator` enforces that exactly one of `voice_id`/`ref_audio` is present; `is_clone` distinguishes them. An optional `seed` pins the server's RNG so a voice stays consistent across chunks and re-encodes; an optional `temperature` lowers sampling variance to flatten text-driven expressiveness. Both are sent per request and serialized with the voice into the manifest (their effect depends on the server honoring those parameters). `--voice-id` and `--voice-ref-audio` are the mutually-exclusive CLI entry points.
