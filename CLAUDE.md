...
**Voice model.** `Voice` (`voice.py`) is an empty Pydantic base; `QwenVoice` (`qwen/voice.py`) subclasses it. A voice is either a **preset** (`voice_id`, e.g. `"Ryan"`, with optional `instruct` emotion/style and `language`) or a **clone** (`ref_audio` path, optional `ref_text` transcript for higher-quality ICL mode vs. zero-shot). A `model_validator` enforces that exactly one of `voice_id`/`ref_audio` is present; `is_clone` distinguishes them. `--voice-id` and `--voice-ref-audio` are the mutually-exclusive CLI entry points.
