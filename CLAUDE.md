...
`zaphodvox` is a CLI and Python library that encodes a text file (or a JSON manifest) into synthetic speech audio using a **locally-hosted TTS server**. Two backends: **qwen** (the default lineage) — the [`eddie-tts`](https://github.com/gumptionthomas/eddie-tts) fork of [cornball-ai/qwen3-tts-api](https://github.com/cornball-ai/qwen3-tts-api), serving the open-weight [Qwen3-TTS](https://github.com/QwenLM/Qwen3-TTS) models. Requires Python >=3.10 and a working `ffmpeg` install (used via `pydub`).
...
- **Read:** `resolve_ref()` — absolute as-is, leading `~` expanded, relative resolved against the declaring file's directory. `QwenVoice._base_dir` (a `PrivateAttr`, never serialized) holds that anchor; `main.anchor_voices()` sets it in `read_voices()` and `read_text_manifest()`. A voice from the CLI has no anchor and stays CWD-relative. `QwenVoice.anchor()` no-ops when there is no `ref_audio` — otherwise two identical preset voices read from different files would compare unequal, since Pydantic includes private attrs in `__eq__`.
...
`Encoder.validate_voice()` is a hook (no-op on the ABC; `QwenEncoder` checks the reference file exists) called in a pre-pass at the top of `encode_manifest()`, so a bad path fails on the command line instead of 200 fragments into a book.
...
**This only works because every fragment shares one sample format**, which is why `create_silence()` takes `AudioParams` and a silent fragment can't be written until some speech exists to copy the format from (`Encoder.silence_params()` reads it off a fragment the server actually returned — only the server knows what it emits). Silence used to be written at pydub's default 11025 Hz next to 24 kHz speech, and `pydub` quietly resampled it on concat. **`ffmpeg`'s concat demuxer cannot be trusted to fix a mismatch** — given mixed rates it silently dropped audio from a wav (2.9s of a 4.0s book), even with `-ar` forced. `_concat_wav()` therefore converts any fragment whose format disagrees, one at a time, which is what keeps already-encoded books working.

**An interrupted `--encode` is resumable, and that is a designed property, not luck.** Three things make it so, and all three are load-bearing. (1) `main.save_manifest()` writes the manifest on `KeyboardInterrupt` — `encode_manifest()` fills the manifest in as it goes, so what is in hand at the interrupt already describes exactly how far it got — then prints the `--indexes` spec of what is still missing (`main.resume_indexes()`; note an `--indexes` range is **half-open**, so fragments 2 and 3 are asked for as `2-4`) and exits 130. It writes that manifest **even under `--no-manifest`**: that flag means a *finished* book does not need one, but a half-finished book is unrecoverable without it — the fragments on disk are orphaned audio unless something records which text they are. (2) Silences are written eagerly (below). (3) `concat_files()` refuses to stitch a book with a fragment missing (below). Take any one away and an interrupt quietly costs you either the whole run or the pauses in it.

**A silent fragment is written as soon as the format is known, and no later.** `encode_manifest()` resolves `silence_params()` on the first blank fragment it reaches and writes every subsequent one inline; only a silence with *no speech yet on disk* (a book opening with a blank line) is held back to the end. This is about interrupts, not speed: an encode is resumed with `--indexes` over the fragments that are missing, so a silence the run had merely queued in memory would die with the process and never be come back for — leaving a book whose pauses are gone from everything before the Ctrl-C. Relatedly, **`concat_files()` raises `FileNotFoundError` on a fragment with no audio file** rather than skipping it. It still skips a fragment that exists but is *unreadable* (a corrupt file, warned about per-fragment); what it will not do is treat an absent one as a non-event, which used to turn an interrupted encode into a finished-looking audiobook with holes in it.
